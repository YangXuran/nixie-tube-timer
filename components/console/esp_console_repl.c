...
static esp_err_t esp_console_repl_bt_spp_delete(esp_console_repl_t *repl);
...
esp_err_t esp_console_new_repl_spp(const esp_console_dev_spp_config_t *dev_config, const esp_console_repl_config_t *repl_config, esp_console_repl_t **ret_repl)
{
    esp_err_t ret = ESP_OK;
    esp_console_repl_universal_t *spp_repl = NULL;
    if (!repl_config | !dev_config | !ret_repl) {
        ret = ESP_ERR_INVALID_ARG;
        goto _exit;
    }
    // allocate memory for console REPL context
    spp_repl = calloc(1, sizeof(esp_console_repl_universal_t));
    if (!spp_repl) {
        ret = ESP_ERR_NO_MEM;
        goto _exit;
    }
    ...
    /* Minicom, screen, idf_monitor send CR when ENTER key is pressed */
    esp_vfs_dev_bt_spp_set_rx_line_endings(ESP_LINE_ENDINGS_CR);
    /* Move the caret to the beginning of the next line on '\n' */
    esp_vfs_dev_bt_spp_set_tx_line_endings(ESP_LINE_ENDINGS_CRLF);

    /* Tell VFS to use UART driver */
    esp_vfs_bt_spp_use_driver();

    // initialize console, common part
    ret = esp_console_common_init(repl_config->max_cmdline_length, &spp_repl->repl_com);
    if (ret != ESP_OK) {
        goto _exit;
    }

    // setup history
    ret = esp_console_setup_history(repl_config->history_save_path, repl_config->max_history_len, &spp_repl->repl_com);
    if (ret != ESP_OK) {
        goto _exit;
    }

    // setup prompt
    esp_console_setup_prompt(repl_config->prompt, &spp_repl->repl_com);

    /* Fill the structure here as it will be used directly by the created task. */
    spp_repl->uart_channel = dev_config->spp_fd;
    spp_repl->repl_com.state = CONSOLE_REPL_STATE_INIT;
    spp_repl->repl_com.repl_core.del = esp_console_repl_bt_spp_delete;

    /* Spawn a single thread to run REPL, we need to pass `uart_repl` to it as
     * it also requires the uart channel. */
    if (xTaskCreate(esp_console_repl_task, "console_repl", repl_config->task_stack_size,
                    spp_repl, repl_config->task_priority, &spp_repl->repl_com.task_hdl) != pdTRUE) {
        ret = ESP_FAIL;
        goto _exit;
    }

    *ret_repl = &spp_repl->repl_com.repl_core;
    return ESP_OK;
_exit:
    if (spp_repl) {
        // uart_driver_delete(dev_config->spp_fd);
        free(spp_repl);
    }
    if (ret_repl) {
        *ret_repl = NULL;
...
static esp_err_t esp_console_repl_bt_spp_delete(esp_console_repl_t *repl)
{
    esp_err_t ret = ESP_OK;
    esp_console_repl_com_t *repl_com = __containerof(repl, esp_console_repl_com_t, repl_core);
    esp_console_repl_universal_t *bt_spp_repl = __containerof(repl_com, esp_console_repl_universal_t, repl_com);
    // check if already de-initialized
    if (repl_com->state == CONSOLE_REPL_STATE_DEINIT) {
        ESP_LOGE(TAG, "already de-initialized");
        ret = ESP_ERR_INVALID_STATE;
...
static void esp_console_repl_task(void *args)
{
    ...
    char path[CONSOLE_PATH_MAX_LEN] = { 0 };
    snprintf(path, CONSOLE_PATH_MAX_LEN, "/dev/spp");
