...
static void bt_spp_tx_char(int fd, int c);
static int bt_spp_rx_char(int fd);
...
typedef struct {
    ...
    int64_t last_tx_ts;
} vfs_bt_spp_context_t;

// If the context should be dynamically initialized, remove this structure
// and point s_ctx to allocated data.
static vfs_bt_spp_context_t s_ctx = {
    .peek_char = NONE,
    .tx_mode = DEFAULT_TX_MODE,
    .rx_mode = DEFAULT_RX_MODE,
    .tx_func = bt_spp_tx_char,
    .rx_func = bt_spp_rx_char
};

extern int esp_spp_data_write(uint32_t handle, int len, uint8_t* p_data);

static int bt_spp_open(const char* path, int flags, int mode)
{
...
extern int esp_spp_data_read(uint32_t handle, int len, uint8_t* p_data, TickType_t timeout);
static int bt_spp_rx_char(int fd)
{
    uint8_t c;
    int l = esp_spp_data_read(fd, 1, &c, 0);
    if (l == 0) {
        return NONE;
    }
    return c;
...
static ssize_t bt_spp_write(int fd, const void* data, size_t size)
{
 ...
 */
static int bt_spp_read_char(int fd)
{
...
static void bt_spp_return_char(int fd, int c)
{
    assert(s_ctx.peek_char == NONE);
    s_ctx.peek_char = c;
}

static ssize_t bt_spp_read(int fd, void* data, size_t size)
{
    char* data_c = (char*)data;
    size_t received = 0;
    _lock_acquire_recursive(&s_ctx.read_lock);
    while (received < size) {
        int c = bt_spp_read_char(fd);
        if (c == '\r') {
            if (s_ctx.rx_mode == ESP_LINE_ENDINGS_CR) {
                c = '\n';
            } else if (s_ctx.rx_mode == ESP_LINE_ENDINGS_CRLF) {
                /* look ahead */
                int c2 = bt_spp_read_char(fd);
                if (c2 == NONE) {
                    /* could not look ahead, put the current character back */
                    bt_spp_return_char(fd, c);
                    break;
                }
                if (c2 == '\n') {
...
static int bt_spp_fstat(int fd, struct stat* st)
{
...
static int bt_spp_close(int fd)
{
    return 0;
}

static int bt_spp_fcntl(int fd, int cmd, int arg)
{
    int result = 0;
    if (cmd == F_GETFL) {
        result |= O_RDWR;
...
static int bt_spp_fsync(int fd)
{
    _lock_acquire_recursive(&s_ctx.write_lock);
    /* TODO
    usb_serial_jtag_ll_txfifo_flush();
...
static int bt_spp_tcsetattr(int fd, int optional_actions, const struct termios* p)
{
    if (p == NULL) {
        errno = EINVAL;
        return -1;
...
static int bt_spp_tcgetattr(int fd, struct termios* p)
{
    if (p == NULL) {
        errno = EINVAL;
        return -1;
...
static int bt_spp_tcdrain(int fd)
{
    bt_spp_fsync(fd);
    return 0;
}

static int bt_spp_tcflush(int fd, int select)
{
...
void esp_vfs_dev_bt_spp_set_tx_line_endings(esp_line_endings_t mode)
{
    s_ctx.tx_mode = mode;
}

void esp_vfs_dev_bt_spp_set_rx_line_endings(esp_line_endings_t mode)
{
    s_ctx.rx_mode = mode;
}
...
static const esp_vfs_t vfs = {
    ...
    .fsync = &bt_spp_fsync,
#ifdef CONFIG_VFS_SUPPORT_TERMIOS
    .tcsetattr = &bt_spp_tcsetattr,
    .tcgetattr = &bt_spp_tcgetattr,
    .tcdrain = &bt_spp_tcdrain,
    .tcflush = &bt_spp_tcflush,
#endif // CONFIG_VFS_SUPPORT_TERMIOS
};

const esp_vfs_t* esp_vfs_bt_spp_get_vfs(void)
{
    return &vfs;
}

esp_err_t esp_vfs_dev_bt_spp_register(void)
{
    // "/dev/usb_serial_jtag" unfortunately is too long for vfs
    return esp_vfs_register("/dev/spp", &vfs, NULL);
}
...
static int btspp_rx_char_via_driver(int fd)
{
    uint8_t c;
    TickType_t timeout = s_ctx.non_blocking ? 0 : portMAX_DELAY;
    int n = esp_spp_data_read(fd, 1, &c, timeout);
    if (n <= 0) {
        return NONE;
    }
    return c;
...
static void btspp_tx_char_via_driver(int fd, int c)
{
    char ch = (char)c;
    esp_spp_data_write(fd, 1, (uint8_t*)&ch);
}

void esp_vfs_bt_spp_use_nonblocking(void)
{
    _lock_acquire_recursive(&s_ctx.read_lock);
    _lock_acquire_recursive(&s_ctx.write_lock);
    s_ctx.tx_func = bt_spp_tx_char;
    s_ctx.rx_func = bt_spp_rx_char;
    _lock_release_recursive(&s_ctx.write_lock);
    _lock_release_recursive(&s_ctx.read_lock);
}

void esp_vfs_bt_spp_use_driver(void)
{
    _lock_acquire_recursive(&s_ctx.read_lock);
    _lock_acquire_recursive(&s_ctx.write_lock);
    s_ctx.tx_func = btspp_tx_char_via_driver;
    s_ctx.rx_func = btspp_rx_char_via_driver;
    _lock_release_recursive(&s_ctx.write_lock);
    _lock_release_recursive(&s_ctx.read_lock);
}
